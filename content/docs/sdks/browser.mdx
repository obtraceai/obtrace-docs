---
title: Browser SDK
description: Frontend telemetry with session replay, Web Vitals, and error capture
---
...
export const ob = initBrowserSDK({
  ...
  ingestBaseUrl: import.meta.env.VITE_OBTRACE_INGEST_URL,
...
After calling `initBrowserSDK`, the SDK installs several hooks with zero additional code from you:
...
`instrumentFetch()` returns a fetch wrapper that auto-creates spans for every outbound HTTP request. It records method, URL, status code, and duration. It also propagates trace context and session ID to your backend, and captures network requests into the session replay.
...
| `serviceName` | `string` | Yes | - | Identifies this frontend app in the Obtrace UI |
