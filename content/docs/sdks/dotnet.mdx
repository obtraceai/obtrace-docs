---
title: .NET SDK
description: Plug-and-play observability for .NET services — Console output and outbound HTTP captured automatically, with explicit APIs for metrics and traces.
---
...
After creating an `ObtraceClient`, the SDK intercepts `Console` output and outbound `HttpClient` calls. You don't need to write any extra code for this:
...
| **Outbound HttpClient calls** | `HttpClient` requests are instrumented with spans containing method, URL, status, and duration, plus `traceparent` header injection | No |
...
`AddObtrace()` registers `ObtraceClient` as a singleton, adds request instrumentation middleware, and hooks into `ApplicationStopping` to flush on shutdown.
