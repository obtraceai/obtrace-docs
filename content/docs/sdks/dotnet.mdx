---
title: .NET SDK
description: Plug-and-play observability for .NET services — Console output captured automatically, with explicit APIs for metrics and traces.
---
...
After creating an `ObtraceClient`, the SDK intercepts `Console` output. You don't need to write any extra code for this:

| What | How | Example in Obtrace UI |
|------|-----|----------------------|
| **Console.WriteLine()** | `Console.Out` writes are intercepted and sent as `info` logs | `info: "Processing order batch 42"` |
| **Console.Error.WriteLine()** | `Console.Error` writes are intercepted and sent as `error` logs | `error: "Failed to connect to database"` |
| **Shutdown flush** | `IAsyncDisposable` flushes remaining data on dispose | Ensures no data is lost on clean shutdown |

To disable console auto-capture, set `AutoCaptureConsole = false` in your config.

There are no diagnostic listeners, no HTTP handler wrappers, and no background collectors beyond console interception. Use the ASP.NET Core middleware (see [Framework Integration](#framework-integration)) or the explicit APIs below for HTTP spans and business telemetry.

## Optional: Custom Telemetry

The automatic console capture covers basic observability. For **business-specific** events, use the SDK methods directly.
