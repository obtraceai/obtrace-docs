---
title: Python SDK
description: Plug-and-play observability for Python — logging and outbound HTTP captured automatically, metrics and traces for what matters
---
...
| **Outbound HTTP calls** | `requests` and `httpx` are patched with span creation and trace propagation | No |
...
Every outbound call made through `requests.get()`, `requests.post()`, `httpx.get()`, `httpx.post()`, etc. is automatically wrapped with a span containing method, URL, status code, and duration. Trace context (`traceparent` header) is injected into outbound requests so downstream services can correlate traces.
...
For inbound HTTP request spans, add the middleware:
...
This auto-captures every inbound request as a span with method, path, status, and duration.
