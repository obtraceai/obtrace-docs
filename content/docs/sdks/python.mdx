...
The Python SDK is a thin wrapper that configures OpenTelemetry with the Obtrace ingest endpoint. Installing it with the `[auto]` extra gives you every OTel auto-instrumentation for Python with zero extra code.

## Installation

```bash
pip install obtrace-sdk-python[auto]
```

The `[auto]` extra installs all available OpenTelemetry instrumentations. Use `pip install obtrace-sdk-python` for the base package without auto-instrumentations.
...
With `obtrace-sdk-python[auto]`, the SDK enables all available OpenTelemetry instrumentations for your installed packages:
...
With `obtrace-sdk-python[auto]`, inbound HTTP instrumentation is automatic via OpenTelemetry. No middleware registration is needed for Django, Flask, or FastAPI.
...
- [ ] Database queries (psycopg2, sqlalchemy, etc.) show auto-generated spans when using `obtrace-sdk-python[auto]`
