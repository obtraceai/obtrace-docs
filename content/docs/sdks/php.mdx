---
title: PHP SDK
description: Plug-and-play observability for PHP services — errors and exceptions captured automatically, with explicit APIs for metrics and traces.
---
...
After creating an `ObtraceClient`, the SDK registers error handlers, exception handlers, and a shutdown function. You don't need to write any extra code for this:

| What | How | Example in Obtrace UI |
|------|-----|----------------------|
| **PHP errors** | `set_error_handler` intercepts errors with level mapping: `E_NOTICE` -> `info`, `E_WARNING` -> `warn`, `E_ERROR` -> `error` | `warn: "Undefined variable $user in checkout.php:42"` |
| **Uncaught exceptions** | `set_exception_handler` catches unhandled exceptions and logs them at `fatal` level | `fatal: "PDOException: Connection refused"` |
| **Shutdown flush** | `register_shutdown_function` flushes buffered telemetry when the PHP process exits, covering both normal completion and fatal errors | Ensures no data is lost |

There are no request interceptors or database query listeners. Use the Laravel or Symfony middleware (see [Framework Integration](#framework-integration)) for HTTP spans, and the explicit APIs below for business telemetry.

## Optional: Custom Telemetry

The automatic error and exception capture covers basic observability. For **business-specific** events, use the SDK methods directly.
