---
title: PHP SDK
description: Plug-and-play observability for PHP services — errors, exceptions, and outbound HTTP captured automatically, with explicit APIs for metrics and traces.
---
...
After creating an `ObtraceClient`, the SDK registers error handlers, exception handlers, HTTP instrumentation, and a shutdown function. You don't need to write any extra code for this:
...
| **Outbound HTTP (cURL)** | `curl_exec` calls are instrumented with spans containing method, URL, status, and duration, plus `traceparent` header injection | No |
| **Outbound HTTP (Guzzle)** | Guzzle middleware is auto-registered when Guzzle is detected, creating spans and injecting trace headers | No |
...
'obtrace' => [
    ...
    'ingest_url' => env('OBTRACE_INGEST_BASE_URL'),
],
```
