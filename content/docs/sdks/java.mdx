---
title: Java SDK
description: Manual instrumentation for JVM services — logs, metrics, and traces sent to Obtrace via OTLP.
---
...
Almost nothing. The Java SDK does not auto-instrument HTTP servlets, JDBC queries, thread pools, or JMX metrics. The only automatic behavior is the shutdown hook (enabled by default) that flushes pending telemetry when the JVM exits.

If you create a client and never call `log`, `metric`, or `span`, no telemetry reaches Obtrace beyond the shutdown hook firing on an empty queue. Every signal requires an explicit call. This is by design: you control exactly what gets sent, with no hidden bytecode manipulation or agent overhead.
