---
title: Java SDK
description: Plug-and-play observability for JVM services — java.util.logging captured automatically, with explicit APIs for metrics and traces.
---
...
public class Main {
    public static void main(String[] args) {
        ...
        try (ObtraceClient client = new ObtraceClient(cfg)) {
...
`ObtraceClient` implements `AutoCloseable`, so use try-with-resources to ensure the internal executor shuts down cleanly. The default config also registers a JVM shutdown hook that calls `flush()` before exit. Disable it with `cfg.registerShutdownHook = false` if you manage lifecycle yourself.
...
|------|-----|----------------------|
| **All java.util.logging output** | JUL log records are intercepted and mapped to Obtrace levels: `FINE` -> `debug`, `INFO` -> `info`, `WARNING` -> `warn`, `SEVERE` -> `error` | `warn: "Connection pool exhausted"` |
| **Shutdown flush** | A JVM shutdown hook flushes pending telemetry before exit (disable with `cfg.registerShutdownHook = false`) | Ensures no data is lost on clean shutdown |

There is no bytecode manipulation, no Java agent, and no hidden overhead. HTTP servlets, JDBC queries, thread pools, and JMX metrics are **not** auto-instrumented. Use the Spring Boot integration (see [Framework Integration](#framework-integration)) or the explicit APIs below for those.
...
ctx.attrs.put("order_id", orderId);
ctx.attrs.put("provider", "stripe");
ctx.attrs.put("decline_code", resp.getDeclineCode());
...
The `ctx` parameter is optional (pass `null` to skip). When provided, it attaches structured attributes to the log entry. Use `attrs` for custom key-value pairs, and the built-in fields (`traceId`, `spanId`, `method`, `endpoint`, `statusCode`) to correlate logs with traces.
...
ObtraceContext ctx = new ObtraceContext();
ctx.method = "POST";
ctx.endpoint = "/api/checkout";
client.metric("http.server.duration", 142.5, "ms", ctx);
