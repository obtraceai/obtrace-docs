---
title: Go SDK
description: Manual instrumentation for Go services — logs, metrics, and traces sent to Obtrace via OTLP.
---
...
Nothing. The Go SDK has zero auto-instrumentation. It does not capture HTTP requests, goroutine stats, runtime metrics, or anything else automatically.

If you create a client and never call `Log`, `Metric`, or `Span`, no telemetry reaches Obtrace. Every signal requires an explicit call. This is by design: you control exactly what gets sent, with no hidden overhead.
